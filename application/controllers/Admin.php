...
class Admin extends CI_Controller
{
    ...
    public function frontend_settings($param1 = "")
    {
        ...
        if ($param1 == 'banner_image_update') {
            ...
            // $res=$this->create_Multiimage("banner_image","uploads/system/");
    ...
    public function submitBanner($param1 = "", $param2 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($param1 == 'add') {
            ...
            $response = $this->crud_model->add_banner();
            if ($response) {
                $this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
            } else {
                $this->session->set_flashdata('error_message', get_phrase('banner_not_added'));
            }
            redirect(site_url('admin/banner'), 'refresh');
        } elseif ($param1 == "edit") {

            $response = $this->crud_model->edit_banner($param2);
            if ($response) {
                $this->session->set_flashdata('flash_message', get_phrase('banner_updated_successfully'));
            } else {
                $this->session->set_flashdata('error_message', get_phrase('banner_not_updated'));
            }
            redirect(site_url('admin/banner'), 'refresh');
        } elseif ($param1 == "delete") {
