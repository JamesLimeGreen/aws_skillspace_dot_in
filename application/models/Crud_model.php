...
class Crud_model extends CI_Model
{
    ...
    public function update_peach_payment_settings()
    {
        ...
        $peach_payment['active'] = $this->input->post('peach_payment_active');
        $peach_payment['testmode'] = $this->input->post('testmode');
        $peach_payment['test_access_token'] = $this->input->post('test_access_token');
        $peach_payment['test_entity_id'] = $this->input->post('test_entity_id');
        ...
        $data['value'] = html_escape($this->input->post('peach_payment_currency'));
        $this->db->where('key', 'peach_payment_currency');
        $this->db->update('settings', $data);
    }
    ...
    public function course_purchase($user_id, $method, $amount_paid, $param1 = "", $param2 = "")
    {
        ...
        foreach ($purchased_courses as $purchased_course) {
            ...
            if ($applied_coupon) {
                ...
                $discount_amount = ($data['amount'] * $coupon_details['discount_percentage']) / 100;
                $data['amount'] = $data['amount'] - $discount;
                $data['coupon'] = $applied_coupon;
                $data['discount'] = $coupon_details['discount_percentage'];
    ...
    {
        return $this->db->get_where('tbl_banner', array('banner_id' => $id));
    }
    ...
    public function edit_banner($param1)
    {
        ...
        $this->db->where('heading', $data['heading']);
        $this->db->or_where('slug', $data['slug']);
        $previous_data = $this->db->get('tbl_banner')->result_array()[0];
