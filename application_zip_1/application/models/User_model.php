...
public function update_instructor_peach_payment_settings($user_id = '') {
        // Update Peach Payment keys
        $peach_payment_info = array();
        $peach_payment_keys = array(
            'live_entity_id' => html_escape($this->input->post('live_entity_id')),
            'live_access_token' => html_escape($this->input->post('live_access_token'))
        );
        array_push($peach_payment_info, $peach_payment_keys);
        $data['peach_payment_keys'] = json_encode($peach_payment_info);
        $this->db->where('id', $user_id);
        $this->db->update('users', $data);
    }
