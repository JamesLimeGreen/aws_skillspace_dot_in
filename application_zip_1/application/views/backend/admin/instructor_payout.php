...
$peach_payment_keys          = json_decode($pending_payout_user_data['peach_payment_keys'], true);
                                        ?>
                                        <tr class="gradeU">
                                            <td> <?php echo ++$key; ?> </td>
                                            <td>
                                            ...
                                            <?php if ( $peach_payment_keys[0]['live_entity_id'] != "" && $peach_payment_keys[0]['live_access_token'] != ""): ?>
                                                    
                                                  <form action="<?php echo site_url('admin/peach_payment_checkout_for_instructor_revenue'); ?>" method="post">
                                                    <input type="hidden" name="amount_to_pay"        value="<?php echo $pending_payout['amount']; ?>">
                                                    <input type="hidden" name="payout_id"            value="<?php echo $pending_payout['id']; ?>">
                                                    <input type="hidden" name="instructor_name"      value="<?php echo $pending_payout_user_data['first_name'].' '.$pending_payout_user_data['last_name']; ?>">

                                                    <input type="hidden" name="live_entity_id" value="<?php echo $peach_payment_keys[0]['live_entity_id']; ?>">

                                                    <input type="hidden" name="live_access_token" value="<?php echo $peach_payment_keys[0]['live_access_token']; ?>">

                                                    <input type="submit" class="btn btn-outline-info btn-sm btn-rounded"        value="Pay With Peach Payment">
                                                  </form>
                                                <?php else: ?>
                                                  <button type="button" class = "btn btn-outline-danger btn-sm btn-rounded" name="button" onclick="alert('<?php echo get_phrase('this_instructor_has_not_provided_valid_peach_payment_credentials'); ?>')">Pay With Peach Payment</button>
                                                <?php endif; ?>
