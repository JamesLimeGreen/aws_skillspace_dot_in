...
$peach_payment_settings = $this->db->get_where('settings', array('key' => 'peach_payment_keys'))->row()->value;
$peach_payment = json_decode($peach_payment_settings);
?>
<!-- start page title -->
<div class="row ">
    <div class="col-xl-12">
...
<div class="col-md-12">
    <div class="card">
        <div class="card-body">
            <h4 class="header-title"><p><?php echo get_phrase('setup_peach_payment_settings'); ?></p></h4>
            <form class="" action="<?php echo site_url('admin/payment_settings/peach_payment_settings'); ?>" method="post" enctype="multipart/form-data">
                <div class="form-group">
                    <label><?php echo get_phrase('active'); ?></label>
                    <select class="form-control select2" data-toggle="select2" id = "stripe_active" name="peach_payment_active">
                        <option value="0" <?php if ($peach_payment[0]->active == 0) echo 'selected';?>> <?php echo get_phrase('no');?></option>
                        <option value="1" <?php if ($peach_payment[0]->active == 1) echo 'selected';?>> <?php echo get_phrase('yes');?></option>
                    </select>
                </div>
                ...
                <div class="form-group">
                    <label><?php echo get_phrase('test_mode'); ?></label>
                    <select class="form-control select2" data-toggle="select2" id = "testmode" name="testmode">
                        <option value="on" <?php if ($peach_payment[0]->testmode == 'on') echo 'selected';?>> <?php echo get_phrase('on');?></option>
                        <option value="off" <?php if ($peach_payment[0]->testmode == 'off') echo 'selected';?>> <?php echo get_phrase('off');?></option>
                    </select>
                </div>

                <div class="form-group">
                    <label><?php echo get_phrase('peach_payment_currency'); ?></label>
                    <select class="form-control select2" data-toggle="select2" id = "peach_payment_currency" name="peach_payment_currency" required>
                        <option value=""><?php echo get_phrase('select_peach_payment_currency'); ?></option>
                        <?php
                        $currencies = $this->crud_model->get_peach_payment_supported_currencies();
                        foreach ($currencies as $currency):?>
                        <option value="<?php echo $currency['code'];?>"
                            <?php if (get_settings('peach_payment_currency') == $currency['code'])echo 'selected';?>> <?php echo $currency['code'];?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="form-group">
                <label><?php echo get_phrase('test_access_token'); ?></label>
                <input type="text" name="test_access_token" class="form-control" value="<?php echo $peach_payment[0]->test_access_token;?>" required />
            </div>

            <div class="form-group">
                <label><?php echo get_phrase('test_entity_id'); ?></label>
                <input type="text" name="test_entity_id" class="form-control" value="<?php echo $peach_payment[0]->test_entity_id;?>" required />
            </div>

            <div class="form-group">
                <label><?php echo get_phrase('live_access_token'); ?></label>
                <input type="text" name="live_access_token" class="form-control" value="<?php echo $peach_payment[0]->live_access_token;?>" required />
            </div>

            <div class="form-group">
                <label><?php echo get_phrase('live_entity_id'); ?></label>
                <input type="text" name="live_entity_id" class="form-control" value="<?php echo $peach_payment[0]->live_entity_id;?>" required />
            </div>

            <div class="row justify-content-md-center">
                <div class="form-group col-md-6">
                    <button class="btn btn-block btn-primary" type="submit">Update Peach Payment Keys</button>
                </div>
            </div>
        </form>
    </div>
