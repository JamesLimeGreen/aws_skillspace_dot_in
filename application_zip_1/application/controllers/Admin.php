...
class Admin extends CI_Controller {
    ...
    public function payment_settings($param1 = "") {
        ...
        if ($param1 == 'peach_payment_settings') {
            $this->crud_model->update_peach_payment_settings();
            redirect(site_url('admin/payment_settings'), 'refresh');
        }
...
public function peach_payment_checkout_for_instructor_revenue() {
        if ($this->session->userdata('admin_login') != 1)
        redirect(site_url('login'), 'refresh');

        $page_data['amount_to_pay']         = $amount = $this->input->post('amount_to_pay');
        $page_data['payout_id']            = $this->input->post('payout_id');
        $page_data['instructor_name']       = $this->input->post('instructor_name');
        $page_data['live_access_token']  = $this->input->post('live_access_token');
        $page_data['live_entity_id']  = $this->input->post('live_entity_id');
        $this->session->set_userdata('payout_id', $this->input->post('payout_id'));
        // BEFORE, CHECK PAYOUT AMOUNTS ARE VALID
        $payout_details = $this->crud_model->get_payouts($page_data['payout_id'], 'payout')->row_array();
        if ($payout_details['amount'] == $page_data['amount_to_pay'] && $payout_details['status'] == 0) {
      ...
      if($peach_payment[0]->testmode == 'on'){
        $entityId = trim($this->input->post('live_entity_id'));
        $accessToken = trim( $this->input->post('live_access_token'));
    ...
    $this->load->view('backend/admin/peach_payment_checkout_for_instructor_revenue', $page_data);
        }else{
            $this->session->set_flashdata('error_message', get_phrase('invalid_payout_data'));
            redirect(site_url('admin/instructor_payout'), 'refresh');
        }
...
// PEACH CHECKOUT ACTIONS
    public function peach_payment() {
       if(!empty($_GET['resourcePath'])){
            $payout_id = $this->session->userdata('payout_id');
            $this->crud_model->update_payout_status($payout_id, 'Peach Payment');
            $this->session->set_flashdata('flash_message', get_phrase('payout_updated_successfully'));
            $this->session->set_userdata('payout_id', '');
        }
